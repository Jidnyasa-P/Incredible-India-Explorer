...
(function() {
  ...
  const statsData = [
    { label: 'Space missions', value: '100+', detail: 'India has built a deep track record across Earth orbit, lunar, Mars, and solar science missions.' },
    { label: 'Nobel-winning discovery', value: 'Raman Effect', detail: "C. V. Raman's 1928 discovery reshaped physics and earned the 1930 Nobel Prize." },
    { label: 'Satellites launched', value: '400+', detail: 'Indian launch systems have carried domestic and international payloads into orbit for decades.' },
    { label: 'Research institutions', value: '70+', detail: 'Premier labs, councils, and universities continue to power discovery across the country.' }
  ];
  ...
  const categoryLabels = { all: 'All', space: 'Space', physics: 'Physics', medicine: 'Medicine', agriculture: 'Agriculture', mathematics: 'Mathematics', biology: 'Biology' };
  ...
  function initSciencePage() {
    ...
    if (!statsGrid || !isroGrid || !scientistGrid || !searchInput || !filterButtons.length || !timelineGrid || !timelineDetail || !modal || !modalClose) return;
    ...
    searchInput.addEventListener('input', () => renderScientists());
    ...
    function renderStats() {
      statsGrid.innerHTML = statsData.map(stat => `<article class="science-stat-card glass-card"><span class="science-stat-label">${stat.label}</span><strong class="science-stat-value">${stat.value}</strong><p class="science-stat-detail">${stat.detail}</p></article>`).join('');
    }

    function renderIsroCards() {
      isroGrid.innerHTML = isroData.map(item => `<article class="science-isro-card glass-card" data-mission-type="${item.category}"><div class="science-isro-card-top"><div class="science-mission-avatar science-avatar-frame" aria-hidden="true"><img class="science-avatar-image science-avatar-image-contain" src="${item.image}" alt="${item.title}" loading="lazy" decoding="async"></div><div class="science-isro-card-head"><span class="science-badge ${item.category}">${categoryLabels[item.category] || 'Space'}</span><span class="science-mission-year">${item.year}</span></div></div><h3>${item.title}</h3><p class="science-mission-summary">${item.summary}</p><p class="science-mission-contribution">${item.contribution}</p><div class="science-chip-row">${item.highlights.map(h => `<span class="science-chip">${h}</span>`).join('')}</div></article>`).join('');
    }

    function renderScientists() {
      const query = searchInput.value.trim().toLowerCase();
      const filtered = scientistData.filter(s => {
        const matchesFilter = activeFilter === 'all' || s.category === activeFilter;
        const matchesSearch = !query || [s.name, s.subtitle, s.achievement, s.contribution, s.story, s.category, ...(s.highlights || [])].join(' ').toLowerCase().includes(query);
        return matchesFilter && matchesSearch;
      });

      scientistGrid.innerHTML = '';
      if (filtered.length === 0) {
        scientistGrid.innerHTML = `<div class="science-empty-state glass-card"><h3>No scientists found</h3><p>Try a different search term or switch back to All Fields.</p><button type="button" class="btn btn-primary" id="science-reset-filters">Show All Scientists</button></div>`;
        document.getElementById('science-reset-filters')?.addEventListener('click', () => { searchInput.value = ''; activeFilter = 'all'; setActiveFilterButton('all'); renderScientists(); });
        return;
      }
      ...
      filtered.forEach(scientist => {
        ...
        card.innerHTML = `<div class="science-scientist-card-head"><div class="science-scientist-avatar science-avatar-frame ${scientist.category} ${scientist.id}" aria-hidden="true"><img class="science-avatar-image science-avatar-image-cover" src="${scientist.image}" alt="${scientist.name}" loading="lazy" decoding="async"></div><div class="science-scientist-title"><span class="science-badge ${scientist.category}">${categoryLabels[scientist.category] || scientist.category}</span><h3>${scientist.name}</h3><p>${scientist.subtitle}</p></div></div><p class="science-scientist-achievement">${scientist.achievement}</p><p class="science-scientist-contribution">${scientist.contribution}</p><div class="science-chip-row">${scientist.highlights.map(item => `<span class="science-chip">${item}</span>`).join('')}</div><div class="science-scientist-footer"><span class="science-card-note">Click to explore achievements</span><button type="button" class="btn btn-secondary science-view-btn">Open Details</button></div>`;
        card.addEventListener('click', () => openModal(scientist, card));
        card.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openModal(scientist, card); } });
        scientistGrid.appendChild(card);
      });
    }

    function renderTimeline() {
      timelineGrid.innerHTML = timelineData.map(item => `<button type="button" class="science-timeline-item glass-card ${item.category}" data-science-timeline-id="${item.id}" aria-pressed="false"><span class="timeline-year">${item.year}</span><span class="science-badge ${item.category}">${categoryLabels[item.category] || 'Science'}</span><h3>${item.title}</h3><p>${item.summary}</p></button>`).join('');
    }

    function renderTimelineDetail(item) {
      timelineDetail.innerHTML = `<div class="timeline-detail-head"><span class="science-badge ${item.category}">${categoryLabels[item.category] || 'Science'}</span><span class="timeline-detail-year">${item.year}</span></div><h3>${item.title}</h3><p>${item.detail}</p>`;
    }

    function setActiveFilterButton(filterValue) {
      filterButtons.forEach(btn => { const isActive = (btn.getAttribute('data-science-filter') || 'all') === filterValue; btn.classList.toggle('active', isActive); btn.setAttribute('aria-pressed', String(isActive)); });
    }

    function setActiveTimelineButton(activeButton) {
      const buttons = timelineGrid.querySelectorAll('[data-science-timeline-id]');
      buttons.forEach(btn => { const isActive = activeButton ? btn === activeButton : false; btn.classList.toggle('active', isActive); btn.setAttribute('aria-pressed', String(isActive)); });
    }

    function openModal(scientist, trigger) {
      lastFocusedTrigger = trigger || document.activeElement; isModalOpen = true;
      modalCategory.className = `science-badge ${scientist.category}`; modalCategory.textContent = categoryLabels[scientist.category] || scientist.category;
      modalTitle.textContent = scientist.name; modalSubtitle.textContent = scientist.subtitle; modalStory.textContent = scientist.story;
      modalHighlights.innerHTML = scientist.highlights.map(item => `<li>${item}</li>`).join('');
      modalStats.innerHTML = scientist.stats.map(stat => `<div class="modal-stat"><span class="modal-stat-label">${stat.label}</span><span class="modal-stat-value">${stat.value}</span></div>`).join('');
  ...
  window.IIE.SciencePage = { init: initSciencePage };
})();
